# Aggregation by 5 with +150% change percentage for out-of-control shifts Poisson
...
code=function(number, comm, new.within, within, between){
  ...
  for(t in 1:2000){
    ...
    while(w.loop.indi==0 & agg.w.loop.indi==0){
      ...
      agg.m.tilda=agg.m.tilda2=agg.m.tilda3=rep(NA, (860)/5-20)
      s.tilda=s.tilda2=s.tilda3=rep(NA, 860-40)
      agg.s.tilda=agg.s.tilda2=agg.s.tilda3=rep(NA, (860)/5-40)
      agg.e=agg.e.bernoulli=list()
      # This specifies aggregation level in the simulation
      agg.index=seq(5, 860, by=5)
      for(i in 1:(860)){
        ...
        if(i %in% agg.index){
          agg.data=sequential2(agg.e[(i-4):i], agg.e.bernoulli[(i-4):i])
          ...
          if(i > 100){
            agg.std=standardization(agg.degree[,(i/5-20):(i/5)],agg.fst[,(i/5-20):(i/5)],agg.snd[,(i/5-20):(i/5)], n=number)
            agg.m.tilda[i/5-20]=agg.std[[1]]
            agg.m.tilda2[i/5-20]=agg.std[[2]]
            agg.m.tilda3[i/5-20]=agg.std[[3]]
          }
          if(i > 200){
            agg.priebe=standardization2(agg.m.tilda[(i/5-40):(i/5-20)], agg.m.tilda2[(i/5-40):(i/5-20)], agg.m.tilda3[(i/5-40):(i/5-20)])
            agg.s.tilda[i/5-40]=agg.priebe[[1]]
            agg.s.tilda2[i/5-40]=agg.priebe[[2]]
            agg.s.tilda3[i/5-40]=agg.priebe[[3]]
          }
        }
        ...
        if(check.integer(shift.point/5-40)==T){
          if(length(which(agg.s.tilda[1:(shift.point/5-41)]>4))==0 && length(which(agg.s.tilda2[1:(shift.point/5-41)]>4))==0 && length(which(agg.s.tilda3[1:(shift.point/5-41)]>4))==0){
            agg.w.loop.indi=1
          } else if(length(which(agg.s.tilda[1:floor(shift.point/5-40)]>4))==0 && length(which(agg.s.tilda2[1:floor(shift.point/5-40)]>4))==0 && length(which(agg.s.tilda3[1:floor(shift.point/5-40)]>4))==0){
            agg.w.loop.indi=1
          }
        }
      }
    ...
    if(check.integer(shift.point/5-40)==T){
      if(length(which(agg.s.tilda[1:(shift.point/5-41)]>4))==0 && length(which(agg.s.tilda[(shift.point/5-40):((860)/5-40)]>4))>=1){
        agg.s.tilda.work[t]=1
        agg.detection.timepoint[t]=which(agg.s.tilda>4)[1]
      }else if(length(which(agg.s.tilda2[1:(shift.point/5-41)]>4))==0 && length(which(agg.s.tilda2[(shift.point/5-40):((860)/5-40)]>4))>=1){
        agg.s.tilda.work[t]=1
        agg.detection.timepoint[t]=which(agg.s.tilda2>4)[1]
      }else if(length(which(agg.s.tilda3[1:(shift.point/5-41)]>4))==0 && length(which(agg.s.tilda3[(shift.point/5-40):((860)/5-40)]>4))>=1){
        agg.s.tilda.work[t]=1
        agg.detection.timepoint[t]=which(agg.s.tilda3>4)[1]
      }
    } else {
      if(length(which(agg.s.tilda[1:floor(shift.point/5-40)]>4))==0 && length(which(agg.s.tilda[ceiling(shift.point/5-40):((860)/5-40)]>4))>=1){
        agg.s.tilda.work[t]=1
        agg.detection.timepoint[t]=which(agg.s.tilda>4)[1]
      }else if(length(which(agg.s.tilda2[1:floor(shift.point/5-40)]>4))==0 && length(which(agg.s.tilda2[ceiling(shift.point/5-40):((860)/5-40)]>4))>=1){
        agg.s.tilda.work[t]=1
        agg.detection.timepoint[t]=which(agg.s.tilda2>4)[1]
      }else if(length(which(agg.s.tilda3[1:floor(shift.point/5-40)]>4))==0 && length(which(agg.s.tilda3[ceiling(shift.point/5-40):((860)/5-40)]>4))>=1){
        agg.s.tilda.work[t]=1
        agg.detection.timepoint[t]=which(agg.s.tilda3>4)[1]
      }
    }
...
output2=output
save(output, output2, file = "v 20 aggregation 5 +50per 2comm poisson.RData")
stopCluster(cl2)
