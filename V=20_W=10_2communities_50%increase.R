...
code=function(number, comm, new.within, within, between){
  ...
  for(t in 1:1000){
    ...
    while(w.loop.indi==0 & agg.w.loop.indi==0){
      ...
      agg.m.tilda=agg.m.tilda2=agg.m.tilda3=rep(NA, (860)/10-20)
      s.tilda=s.tilda2=s.tilda3=rep(NA, 860-40)
      agg.s.tilda=agg.s.tilda2=agg.s.tilda3=rep(NA, (860)/10-40)
      agg.e=agg.e.bernoulli=list()
      # This specifies aggregation level in the simulation
      agg.index=seq(10, 860, by=10)
      for(i in 1:(860)){
        ...
        if(i %in% agg.index){
          agg.data=sequential2(agg.e[(i-9):i], agg.e.bernoulli[(i-9):i])
          ...
          if(i > 200){
            agg.std=standardization(agg.degree[,(i/10-20):(i/10)],agg.fst[,(i/10-20):(i/10)],agg.snd[,(i/10-20):(i/10)], n=number)
            agg.m.tilda[i/10-20]=agg.std[[1]]
            agg.m.tilda2[i/10-20]=agg.std[[2]]
            agg.m.tilda3[i/10-20]=agg.std[[3]]
          }
          if(i > 400){
            agg.priebe=standardization2(agg.m.tilda[(i/10-40):(i/10-20)], agg.m.tilda2[(i/10-40):(i/10-20)], agg.m.tilda3[(i/10-40):(i/10-20)])
            agg.s.tilda[i/10-40]=agg.priebe[[1]]
            agg.s.tilda2[i/10-40]=agg.priebe[[2]]
            agg.s.tilda3[i/10-40]=agg.priebe[[3]]
          }
        }
        ...
        if(check.integer(shift.point/10-40)==T){
          if(length(which(agg.s.tilda[1:(shift.point/10-41)]>4))==0 && length(which(agg.s.tilda2[1:(shift.point/10-41)]>4))==0 && length(which(agg.s.tilda3[1:(shift.point/10-41)]>4))==0){
            agg.w.loop.indi=1
          } else if(length(which(agg.s.tilda[1:floor(shift.point/10-40)]>4))==0 && length(which(agg.s.tilda2[1:floor(shift.point/10-40)]>4))==0 && length(which(agg.s.tilda3[1:floor(shift.point/10-40)]>4))==0){
            agg.w.loop.indi=1
          }
        }
      }
    ...
    if(check.integer(shift.point/10-40)==T){
      if(length(which(agg.s.tilda[1:(shift.point/10-41)]>4))==0 && length(which(agg.s.tilda[(shift.point/10-40):((860)/10-40)]>4))>=1){
        agg.s.tilda.work[t]=1
        agg.detection.timepoint[t]=which(agg.s.tilda>4)[1]
      }else if(length(which(agg.s.tilda2[1:(shift.point/10-41)]>4))==0 && length(which(agg.s.tilda2[(shift.point/10-40):((860)/10-40)]>4))>=1){
        agg.s.tilda.work[t]=1
        agg.detection.timepoint[t]=which(agg.s.tilda2>4)[1]
      }else if(length(which(agg.s.tilda3[1:(shift.point/10-41)]>4))==0 && length(which(agg.s.tilda3[(shift.point/10-40):((860)/10-40)]>4))>=1){
        agg.s.tilda.work[t]=1
        agg.detection.timepoint[t]=which(agg.s.tilda3>4)[1]
      }
    } else {
      if(length(which(agg.s.tilda[1:floor(shift.point/10-40)]>4))==0 && length(which(agg.s.tilda[ceiling(shift.point/10-40):((860)/10-40)]>4))>=1){
        agg.s.tilda.work[t]=1
        agg.detection.timepoint[t]=which(agg.s.tilda>4)[1]
      }else if(length(which(agg.s.tilda2[1:floor(shift.point/10-40)]>4))==0 && length(which(agg.s.tilda2[ceiling(shift.point/10-40):((860)/10-40)]>4))>=1){
        agg.s.tilda.work[t]=1
        agg.detection.timepoint[t]=which(agg.s.tilda2>4)[1]
      }else if(length(which(agg.s.tilda3[1:floor(shift.point/10-40)]>4))==0 && length(which(agg.s.tilda3[ceiling(shift.point/10-40):((860)/10-40)]>4))>=1){
        agg.s.tilda.work[t]=1
        agg.detection.timepoint[t]=which(agg.s.tilda3>4)[1]
      }
    }
...
output2=output
save(output, output2, file = "v 20 aggregation 10 +50per 2comm poisson.RData")
stopCluster(cl2)
